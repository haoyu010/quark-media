...
def _cached_poster_exists(poster_local_path: str) -> bool:
    try:
        poster_local_path = (poster_local_path or '').strip()
        if not poster_local_path:
            return False
        if poster_local_path.startswith('/cache/images/'):
            filename = poster_local_path.replace('/cache/images/', '', 1)
        elif poster_local_path.startswith('cache/images/'):
            filename = poster_local_path.replace('cache/images/', '', 1)
        else:
            return False
        root = os.path.abspath(CACHE_IMAGES_DIR)
...
def _clean_poster_search_title(title: str) -> str:
    try:
        title = str(title or '').strip()
        title = re.sub(r'\s+[-–—]\s+S\d{1,2}E.*$', '', title, flags=re.I)
        title = re.sub(r'\s+(?:Season|S)\s*\d{1,2}.*$', '', title, flags=re.I)
...
def _find_tmdb_poster_for_task(tmdb_service, tmdb_id: int, content_type: str = '', title: str = ''):
    media_hint = str(content_type or '').strip().lower()
    prefer_movie = media_hint in ('movie', 'film', '电影')
    media_order = ['movie', 'tv'] if prefer_movie else ['tv', 'movie']

    for media_type in media_order:
        try:
            if media_type == 'movie':
                details = tmdb_service._make_request(f'/movie/{int(tmdb_id)}') or {}
                poster_path = details.get('poster_path') or ''
            else:
                details = tmdb_service.get_tv_show_details(int(tmdb_id)) or {}
                poster_path = tmdb_service.get_poster_path_with_language(int(tmdb_id)) or details.get('poster_path') or ''
            if poster_path:
                return poster_path, details, media_type, int(tmdb_id)
        except Exception:
            continue

    query = _clean_poster_search_title(title)
    if not query:
        return '', {}, '', None
    ...
    for media_type in media_order:
        try:
            if media_type == 'movie':
                candidates = tmdb_service.search_movie_all(query) or []
            else:
                candidates = tmdb_service.search_tv_show_all(query) or []
            for item in candidates:
                poster_path = item.get('poster_path') or ''
                matched_id = item.get('id')
                if poster_path and matched_id:
                    return poster_path, item, media_type, int(matched_id)
        except Exception:
            continue
    return '', {}, '', None


def resolve_calendar_poster_image_path(tmdb_id: int, content_type: str = '', title: str = '') -> str:
    try:
        tmdb_id = int(tmdb_id)
    except Exception:
        return ''
    ...
    try:
        cal_db = CalendarDB()
        show = cal_db.get_show(tmdb_id) or {}
    except Exception:
        cal_db = None
        show = {}

    existing_poster = (show or {}).get('poster_local_path') or ''
    if existing_poster and _cached_poster_exists(existing_poster):
        return existing_poster

    tmdb_api_key = (config_data.get('tmdb_api_key') or '').strip() if isinstance(config_data, dict) else ''
    if not tmdb_api_key:
        return existing_poster if existing_poster else ''

    try:
        tmdb_service = TMDBService(tmdb_api_key, get_poster_language_setting(), request_timeout=6, max_retries=1)
        poster_path, details, media_type, matched_id = _find_tmdb_poster_for_task(tmdb_service, tmdb_id, content_type, title)
        if not poster_path:
            return existing_poster if existing_poster else ''

        is_same_show = bool(matched_id and int(matched_id) == int(tmdb_id))
        is_custom_poster = bool((show or {}).get('is_custom_poster', 0)) if is_same_show else False
        poster_local_path = download_poster_local(
            poster_path,
            int(matched_id or tmdb_id),
            existing_poster if is_same_show else None,
            is_custom_poster,
        )
        if not poster_local_path:
            return existing_poster if existing_poster else ''

        if cal_db and is_same_show:
            try:
                if show:
                    cal_db.update_show_poster(int(tmdb_id), poster_local_path, int(is_custom_poster))
                elif media_type == 'tv':
                    name = details.get('name') or details.get('original_name') or title or str(tmdb_id)
                    ...
                    try:
                        ...
                        if seasons:
                            latest_season_number = max(int(s.get('season_number') or 1) for s in seasons)
                    except Exception:
                        latest_season_number = 1
                    cal_db.upsert_show(int(tmdb_id), name, year, status, poster_local_path, latest_season_number, 0, '', content_type or '')
            except Exception:
                pass
        return poster_local_path
    except Exception as e:
        logging.debug(f"resolve poster failed tmdb_id={tmdb_id}, title={title}: {e}")
        return existing_poster if existing_poster else ''


@app.route("/api/calendar/poster_image/<int:tmdb_id>")
def get_calendar_poster_image(tmdb_id):
    try:
        if not is_login():
            return redirect(url_for('static', filename='images/no-poster.svg'))
        poster_path = resolve_calendar_poster_image_path(
            tmdb_id,
